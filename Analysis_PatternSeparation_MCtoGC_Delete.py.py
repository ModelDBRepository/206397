...
def analysis_pattern_separation_mossy(Trials, mossy, dend):
    ...
    for i_over in xrange(len(overlap)):
        if mossy == 'control':
            ...
            os.chdir('results/MC_to_GC_delete')
...
fig1.savefig('Pattern_Separation_BARS_MC_to_GC_delete_'+str(Ndend)+'.eps', format = 'eps', dpi = 1200)
plt.close(fig1)
...
plt.show(fig2)
fig2.savefig('Pattern_Separation_MC_to_GC_delete_'+str(Ndend)+'.eps', format = 'eps', dpi = 1200)
plt.close(fig2)
...
plt.show(fig3)
fig3.savefig('GC_activity_MC_to_GC_delete_'+str(Ndend)+'.eps', format = 'eps', bbox_inches='tight', dpi = 1200)
plt.close(fig3)
...
mat1 = {'Control':sampleMOSSY, 'MC_to_GC_delete': sampleNO_MOSSY}
np.save("GC_activity_MC_to_GC_delete.npy", mat1)

mat2 = {'Control':output_MOSSY[4], 'MC_to_GC_delete':output_NO_MOSSY[4]}
np.save("Similarity_out_MC_to_GC_delete.npy", mat2)
